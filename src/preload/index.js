...
// 公开给 renderer 的 invoke 频道（白名单收紧）：仅 renderer 实际需要的。
// 其余 channel 走分组 API（localCacheAPI / engineAPI / dbAPI 等）。
...
function safeOn(channel, callback) {
  ...
  const handler = (_event, ...args) => {
    try {
      ...
      // Renderer-side handler threw — surface to main log so we can see it instead of silent swallow
      try {
